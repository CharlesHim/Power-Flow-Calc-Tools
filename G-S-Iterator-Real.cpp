...
double x[10001][10001] = {0};	//x _i ^k
double y[10001] = {0};	//y _i ^k
double a[10001][10001] = {0};	//a _i _j
...
void iterate(void)
{
	double tmp = 0;	//没法直接在 标记1 处的括号内嵌套循环，因此使用临时变量来间接解决 
	...
	if(method)
	{
		...
		for(i=0;i<size;i++)
		{
			if(fabs(x[i][k+1]-x[i][k]) > current_accuracy)
			{
				current_accuracy = fabs(x[i][k+1]-x[i][k]);
			}
		}
	...
	else 
	{
		...
		for(i=0;i<size;i++)
		{
			if(fabs(x[i][k]-x[i][k-1]) > current_accuracy)
			{
				current_accuracy = fabs(x[i][k]-x[i][k-1]);
			}
		}
	}
}
